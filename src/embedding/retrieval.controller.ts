...
export class RetrievalController {
  ...
  @ApiOperation({
    summary:
      'Semantic search — embed query and return top-K courses from Qdrant'
  })
  @ApiOkResponse({ type: RetrieveCoursesResponseDto })
  public async retrieveCourses(
    @Body() body: RetrieveCoursesDto
  ): Promise<RetrieveCoursesResponseDto> {
    const courses = await this.retrieverService.retrieveCourses(
      body.query,
      body.context
    );
    return { courses };
