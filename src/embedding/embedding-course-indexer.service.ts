...
export class CourseEmbeddingIndexerService {
  ...
  public async run(): Promise<void> {
    ...
    this.logger.log(
      `Loaded ${rows.length} rows from v_courses_for_embedding. ${existingHashes.size} points already in Qdrant.`
    );
    ...
    this.logger.log(
      `Skipping ${counters.skippedCount} unchanged courses. Indexing ${rowsToIndex.length} new/changed courses.`
    );
  ...
  private async processRowsBatch(
    ...
    this.logger.debug(
      `Prepared ${preparedBatch.length}/${rowsBatch.length} courses${batchInfo}. Errors: ${counters.errorCount}.`
    );

    if (preparedBatch.length === 0) {
      this.logger.warn(
        `Batch ${batchNumber || 'unknown'} had 0 prepared items, skipping.`
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `✓ Upserted ${points.length} courses to Qdrant${batchInfo}. Total indexed: ${counters.indexedCount}.`
      );
    } catch (error) {
      this.logger.error(
  ...
  private async processPreparedBatchOneByOne(
    ...
    for (const prepared of preparedBatch) {
      ...
      try {
        ...
        if (!firstPoint) {
          throw new Error(
            'No Qdrant point produced for single prepared course.'
          );
        }
