...
export class CourseController {
  ...
  @Get('search')
  @ApiOperation({
    summary: '🟢 Search courses by query string (query = either code or title)'
  })
  @ApiOkResponse({
    description:
      'Returns paginated search results with courses, total count, and hasMore flag (ordererd by code, then title)',
  ...
  @ApiQuery({
    ...
    required: false,
    description:
      'Search query. Leave empty to return all courses (paginated). Ex: LOG'
  })
  @ApiQuery({
    name: 'programCodes',
    type: String,
