...
describe('SessionService', () => {
  ...
  const buildSession = (year: number, trimester: Trimester): Session => ({
    year,
    trimester,
  ...
  describe('getOrCreateSessionFromCode', () => {
    ...
    it.each([25 as unknown as string, '', 'A2', 'A250'])(
      'throws when the session code is not a 3-character string: %p',
      async (sessionCode) => {
        await expect(
