...
import { runCommand, runCommandWithRetry } from './run-command';
...
export default async function globalSetup(): Promise<void> {
  ...
  if (!databaseUrl) {
    throw new Error('DATABASE_URL is missing in globalSetup');
  }

  assertIsTestDatabase(databaseUrl);

  // Ensure schema is up-to-date for the test DB
  await runCommandWithRetry('yarn', ['prisma:migrate-prod'], process.env);
