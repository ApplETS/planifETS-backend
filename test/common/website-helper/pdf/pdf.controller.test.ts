...
describe('PdfController', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: PlanificationCoursService,
          useValue: planificationCoursService
        }
      ]
    }).compile();
  ...
  describe('parseHoraireCoursPdf', () => {
    it('should throw BAD_REQUEST if session or program is missing', async () => {
      await expect(
        controller.parseHoraireCoursPdf(undefined as unknown as string, '7084')
      ).rejects.toThrow(HttpException);
      await expect(
        controller.parseHoraireCoursPdf('20261', undefined as unknown as string)
      ).rejects.toThrow(HttpException);
    });
  ...
  describe('parsePlanificationCoursPdf', () => {
    it('should throw BAD_REQUEST if program is missing', async () => {
      await expect(
        controller.parsePlanificationCoursPdf(undefined as unknown as string)
      ).rejects.toThrow(HttpException);
    });
    ...
    it('should return parsed data on success', async () => {
      ...
      planificationCoursService.parsePdfFromUrl.mockResolvedValueOnce(
        mockResult
      );
      await expect(
        controller.parsePlanificationCoursPdf('7084')
      ).resolves.toEqual(mockResult);
    });

    it('should throw NOT_FOUND if axios error 404', async () => {
      const error = { isAxiosError: true, response: { status: 404 } };
      planificationCoursService.parsePdfFromUrl.mockRejectedValueOnce(
        error as unknown
      );
      await expect(
        controller.parsePlanificationCoursPdf('7084')
